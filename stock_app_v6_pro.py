...
def evaluate_stock_100(df):
    ...
    try:
        ...
        tests = [
            ...
            (c > df['Close'].iloc[-2], "收盤維持連漲"),
            (c < m20 + df['Close'].rolling(20).std().iloc[-1]*2, "尚未觸及布林上軌")
        ]
...
def get_zhuge_advice(score):
    ...
    elif 75 <= score < 90: return "【萬事俱備，只欠東風】技術面全面翻多，均線排列整齊。爆量突破近期高點即是再度發動訊號。"
    elif 60 <= score < 75: return "【草船借箭，蓄勢待發】底部墊高，量能暗湧，主力偷偷吃貨中。適合分批佈局。"
    elif 40 <= score < 60: return "【兩軍對壘，糾纏不清】月線洗盤中，無量無方向。此乃混水摸魚盤，等出量表態再說。"
    elif 20 <= score < 40: return "【空城計現，外強中乾】看似有撐其實極弱。莫因跌深手癢，底在哪還不知道，保命要緊。"
    else: return "【火燒連環船，兵敗如山倒】趨勢走空，下方無支撐。不宜攤平，撤退保資金為上策。"
...
def plot_v6_pro(df, title, days, show_ma1200=False):
    ...
    ax1.plot(df_slice.index, df_slice['Close'], color='white', linewidth=1.2, label='收盤價')
    if show_ma1200:
...
if ticker:
    ...
    if not hist.empty:
        ...
        if q_data:
            ...
            ax_q.set_xlabel("AI 評分"); ax_q.set_ylabel("當日漲跌%"); fig_q.patch.set_alpha(0.0); st.pyplot(fig_q)
