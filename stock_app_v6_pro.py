...
# --- 1. 雲端環境中文字體修正 ---
# 判定是否在 Linux (Streamlit Cloud) 環境，如果是則使用 Noto Sans CJK
if os.name == 'posix':
    plt.rcParams['font.sans-serif'] = ['Noto Sans CJK JP', 'Noto Sans CJK TC', 'Liberation Sans', 'DejaVu Sans']
else:
    # Mac 本機環境使用
...
def evaluate_stock_100(df):
    ...
    try:
        ...
        tests = [
            (c > m20.iloc[-1], "站上月線"), (m20.iloc[-1] > m20.iloc[-5], "月線趨勢向上"),
            (c > m120.iloc[-1] if not np.isnan(m120.iloc[-1]) else False, "站上半年線"),
            (m120.iloc[-1] > m120.iloc[-5] if not np.isnan(m120.iloc[-1]) else False, "趨勢翻正"),
            (50 < rsi < 75, "RSI攻擊區"), (vol > avg_vol * 1.5, "量能顯著放大"),
            (c > m1200.iloc[-1] if not np.isnan(m1200.iloc[-1]) else True, "基期高於五年線"),
...
def plot_advanced_charts(df, title, days, resample_rule):
    ...
    up, dn = ma20 + df_slice['Close'].rolling(20).std()*2, ma20 - df_slice['Close'].rolling(20).std()*2
    ...
    ax2.set_ylabel(f"{'三日' if resample_rule=='3D' else '每週'}量能")
    
    plt.tight_layout()
    return fig

# --- 3. 網頁 UI 介面 ---
st.sidebar.title("🔍 75分潛力名單")
st.sidebar.caption("即時掃描核心追蹤標的")
...
def get_potential_list():
    ...
    for t, n in CORE_LIST.items():
        ...
        if s >= 75: plist.append((n, t.split('.')[0], s))
    return sorted(plist, key=lambda x: x[2], reverse=True)

for name, code, sc in get_potential_list():
    st.sidebar.success(f"🔥 {name} ({code}) : {sc}分")

st.title("🚀 2026 AI 台股決策系統 V6 Flagship")
query_in = st.text_input("輸入代號或中文名稱", "3675")
ticker_code, chinese_name = get_ticker_info(query_in)

if ticker_code:
    with st.spinner('抓取雲端數據中...'):
        ...
        if not hist.empty:
            ...
            st.markdown(f"📅 **查詢日期**：{now_time} | 🕒 **最後收盤日**：{last_date}")
            ...
            with col1:
                ...
                st.pyplot(plot_advanced_charts(hist, "五年長線 (基期評估 + 每週量能)", 1250, 'W'))
            ...
            with col2:
                ...
                ax_q.set_xlim(0, 100); ax_q.set_xlabel("評分"); ax_q.set_ylabel("當日漲跌幅 %")
