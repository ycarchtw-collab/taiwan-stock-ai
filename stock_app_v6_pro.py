...
if os.path.exists('孔明看盤.png'):
    ...
    [data-testid="stAppViewContainer"]::before {{
        ...
        background-color: rgba(0, 0, 0, 0.7); backdrop-filter: blur(5px); z-index: -1;
    }}
    ...
    .analysis-header {{ color: #ff4b4b; font-weight: bold; font-size: 1.1em; }}
    </style>
    """
else:
...
query_in = st.sidebar.text_input("輸入代號或公司名稱", "3675")

st.sidebar.markdown("---")
st.sidebar.subheader("🚩 監控名單")
@st.cache_data(ttl=3600)
def scan_potential():
    p_list = []
...
for name, code, sc in scan_potential():
    st.sidebar.markdown(f'<div style="color:white; padding:5px; border-bottom:1px solid #444;"><b>{name}</b> ({code}) <span style="color:#ff4b4b;">{sc}分</span></div>', unsafe_allow_html=True)
...
if ticker:
    ...
    if not hist.empty:
        ...
        with col1:
            ...
            st.markdown(f"<div style='color:white; background:#333; padding:5px 10px; border-radius:5px; display:inline-block;'>🔴 大盤: {t_lp:,.2f} <span style='color:{t_pct_color};'>({t_pct:+.2f}%)</span></div>", unsafe_allow_html=True)
        ...
        <div class="analysis-text">
            <span class="analysis-header">📊 落點解析說明：</span><br>
            • <b>右上 (強勢攻擊區)：</b> AI 評分高且漲勢強，標的多頭動能極強。<br>
            • <b>右下 (蓄勢待發區)：</b> AI 評分高但今日壓回，具備補漲或反彈潛力。<br>
            • <b>左上 (過熱投機區)：</b> 評分低但今日漲幅大，小心短線噴出後的回檔風險。<br>
...
st.markdown("---")
st.markdown("<p style='color:#FF6666; font-size: 0.8em; text-align: center; font-weight: bold;'>投資一定有風險，投資有賺有賠，申購前應詳閱公開說明書</p>", unsafe_allow_html=True)
