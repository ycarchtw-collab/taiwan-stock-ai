...
def plot_v6_pro(df, title, days, show_long_term=False):
    ...
    ax1.plot(df_slice.index, ma20, color='#FFA500', alpha=0.6, lw=1.2, ls='--', label='月線')
    ...
    ax1.plot(df_slice.index, df['Close'].rolling(240).mean().tail(days), label='年線', color='#A020F0', lw=1.5)
    if show_long_term:
...
# CSS 注入：核心在於讓 [data-testid="stAppViewContainer"] 透明
st.markdown(f"""
    <style>
    /* 1. 設定背景底圖 (偽元素) */
    [data-testid="stAppViewContainer"]::before {{
        ...
        opacity: 0.25 !important; /* 透明度改為 0.25 讓它更明顯一點 */
        z-index: -1;
    }}

    /* 2. 讓主容器透明化，否則黑底會蓋住偽元素圖片 */
    ...
    /* 4. 裝飾性卡片樣式 */
    .zhuge-advice {{ background: rgba(30, 0, 0, 0.85); padding: 25px; border-radius: 12px; border: 2px solid #FF4B4B; color: white; margin-bottom: 20px; }}
    .data-card {{ background: rgba(45, 45, 45, 0.85); padding: 20px; border-radius: 12px; border: 1px solid #555; text-align: center; }}
    h1, h2, h3, h4 {{ color: #FFFFFF !important; text-shadow: 2px 2px 4px #000; }}
    </style>
""", unsafe_allow_html=True)

# 側邊欄
st.sidebar.title("⌨️ 諸葛神算")
query_in = st.sidebar.text_input("輸入股票代號", "3675").upper()
st.sidebar.markdown("---")
...
if ticker:
    ...
    if not hist.empty:
        ...
        st.markdown(f'<div class="zhuge-advice"><b style="color:#FF4B4B; font-size:1.3rem;">📜 諸葛錦囊：</b><br>{get_zhuge_advice(score)}<br><small>依據：{", ".join(tags)}</small></div>', unsafe_allow_html=True)
